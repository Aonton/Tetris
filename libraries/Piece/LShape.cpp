...
LShape::LShape(int c, int x, int y, int s):
                     Tetrominoes(c,x,y,s){
  setBorders();
};
...
bool LShape::draw() const{
  ...
  switch(getCurrentOrient())
  {
     ...
     {   // L piece will look like this ....[]
      ...
      break;
      default:
      {   Serial.println("LShape::draw() - Incorrect Orientation");
      }
...
void LShape::setBorders(){
  ...
  {   case DEGREE0:
      ...
      default:
      {
         Serial.println("LShape::setBorders() - Incorrect Orientation");
      }
